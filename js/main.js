...
import {
  ...
  NORTH_OFFSET_DEG, TYPE_NAMES, px2m, snapAwayFromRefuge,
} from './siteData.js';
...
// 1. 기본 씬
// ────────────────────────────────────────────────────────────
// 폰에서는 화면이 작고 GPU도 약하니 해상도·그림자 품질을 낮춰 부드럽게 돌아가게 한다
...
const renderer = new THREE.WebGLRenderer({
  ...
  stencil: false,          // 스텐실 안 씀 → 대역폭 절약
});
// 레티나에서 2배면 픽셀이 4배. 1.75는 육안 차이 거의 없으면서 부하를 줄인다.
renderer.setPixelRatio(Math.min(devicePixelRatio, IS_PHONE ? 1.5 : 1.75));
renderer.setSize(innerWidth, innerHeight);
renderer.shadowMap.enabled = true;
renderer.shadowMap.type = IS_PHONE ? THREE.PCFShadowMap : THREE.PCFSoftShadowMap;
// 그림자는 태양이 움직일 때만 다시 굽는다 (카메라만 돌릴 땐 재사용 → 품질 동일·부하↓)
renderer.shadowMap.autoUpdate = false;
renderer.outputColorSpace = THREE.SRGBColorSpace;

/** 필요할 때만 그리기 — 유휴 시 CPU/GPU를 거의 쓰지 않는다 */
...
/**
 * 화면 비율에 맞춰 카메라를 뒤로 물린다.
 * 세로로 긴 폰 화면은 가로 시야각이 좁아서, 데스크톱과 같은 거리에 두면 단지가 잘린다.
 */
function frameSite() {
  const SITE_RADIUS = 385;                       // 단지를 감싸는 대략의 반경(m)
  const vFov = THREE.MathUtils.degToRad(camera.fov);
  const hFov = 2 * Math.atan(Math.tan(vFov / 2) * camera.aspect);
  const dist = SITE_RADIUS / Math.tan(Math.min(vFov, hFov) / 2) * 1.05;
...
// 단지 스케일(~1.2km)에서 2048이면 텍셀당 ~0.6m — 건물 그림자는 충분히 선명하고 4096 대비 GPU 부담이 1/4
sunLight.shadow.mapSize.set(IS_PHONE ? 1536 : 2048, IS_PHONE ? 1536 : 2048);
...
const state = {
  ...
  room: 'living',   // 조망·일조 창: living(거실) | master(안방)
  lastFloor: 21,    // 피난층(20) 슬라이더 스킵 방향 기억
...
// 5. 태양
// ────────────────────────────────────────────────────────────
function updateSun(opts = {}) {
  const forceShadow = opts.forceShadow !== false;
...
function syncBuilding() {
  ...
  let f = +$('floorSlider').value;
  if (f > b.floors) f = b.floors;
  f = snapAwayFromRefuge(f, b.floors);
  $('floorSlider').value = String(f);
  state.lastFloor = f;
  $('floorLabel').textContent = `${f}층`;
...
function buildTypeLegend() {
  ...
  const order = ['84A', '84B', '115A', '115B', '123A', '123B', '132A', '137B', '137C', '152A', '152B'];
...
$('floorSlider').addEventListener('input', () => {
  let f = +$('floorSlider').value;
  const top = +$('floorSlider').max;
  // 피난층(20)은 거주 세대가 아니므로 슬라이더가 건너뛰게 한다
  if (f === 20) {
    f = state.lastFloor < 20 ? Math.min(21, top) : 19;
    $('floorSlider').value = String(f);
  }
  state.lastFloor = f;
  $('floorLabel').textContent = `${f}층`;
...
function setRoom(room) {
  state.room = room === 'master' ? 'master' : 'living';
  document.querySelectorAll('#roomRow .preset').forEach((el) => {
    el.classList.toggle('on', el.dataset.room === state.room);
  });
  if (state.viewMode) enterViewMode();
  writeUrl();
}
$('roomRow')?.addEventListener('click', (e) => {
  const btn = e.target.closest('[data-room]');
  if (btn) setRoom(btn.dataset.room);
});
...
$('playBtn').addEventListener('click', () => {
  ...
  if (!state.playing) updateSun({ forceShadow: true });
  else requestRender();
});
...
// 폰에서는 3D가 먼저 보이도록 시트를 접고, 화면이 좁으니 동 번호도 꺼둔다
if (IS_PHONE) {
...
function currentAnchor() {
  ...
  const ho = +$('hoSelect').value;
  return { ...getUnitAnchor(towerMap[id], floor, ho, state.room), id, floor, ho };
}
...
function enterViewMode() {
  ...
  // 창틀은 방(거실/안방) 개구부 폭에 맞춘다. 안방만 발코니 난간.
  const tower = towerMap[a.id];
  if (windowFrame && windowFrame.parent) windowFrame.parent.remove(windowFrame);
  windowFrame = createWindowFrame(a.wid, { hasBalcony: a.hasBalcony });
  tower.add(windowFrame);
  placeWindowFrame(windowFrame, tower, a.floor, a.ho, state.room);

  // 카메라는 창 바깥이 아니라 방 안쪽 눈높이에 둔다
  ...
  const roomLabel = state.room === 'master' ? '안방' : '거실';

  $('viewBadge').innerHTML =
    `<i class="swatch" style="background:${TYPE_COLORS[type] || '#888'}"></i>` +
    `<b>${a.id}동 ${a.floor}층 ${a.ho}호</b> ${TYPE_NAMES[type] || ''} · ` +
    `<b>${roomLabel}</b> · 지상 ${a.floorY.toFixed(0)}m · 창폭 ${a.wid.toFixed(1)}m · ${openText}`;
...
$('analyzeBtn').addEventListener('click', () => {
  ...
  v.innerHTML = ok
    ? `✅ ${a.id}동 ${a.floor}층 ${a.ho}호 ${roomLabel} — 일반적인 일조 기준(09~15시 4시간 또는 연속 2시간)을 만족합니다.`
    : `⚠️ ${a.id}동 ${a.floor}층 ${a.ho}호 ${roomLabel} — 이 날짜엔 일조가 부족합니다. 층을 올리거나 다른 호수와 비교해 보세요.`;
  box.appendChild(v);
});
...
$('compareBtn').addEventListener('click', () => {
  ...
  const rows = [1, 2, 3, 4].map((ho) => {
    const { position, normal, wid } = getUnitAnchor(towerMap[id], floor, ho, state.room);
  ...
  $('compareTitle').textContent = `${id}동 ${floor}층 · ${roomLabel} · ${state.date}`;
  $('compareTable').innerHTML =
    `<tr><th>호</th><th>주택형</th><th>창폭</th><th>총 일조</th><th>09~15시</th><th>정면 개방</th></tr>` +
...
function selectUnit(id, ho, floor) {
  ...
  const safeFloor = snapAwayFromRefuge(floor, topFloor);
  $('bldSelect').value = id;
  syncBuilding();
  $('hoSelect').value = String(ho);
  $('floorSlider').value = String(safeFloor);
  $('floorLabel').textContent = `${safeFloor}층`;
  state.lastFloor = safeFloor;
  markPicked();
  redrawHeatmap();
  writeUrl();
  if (state.viewMode) enterViewMode();
...
function writeUrl() {
  ...
  if (state.room === 'master') p.set('r', 'm');
...
function readUrl() {
  ...
  if (p.has('b')) {
    ...
    if (p.has('f')) {
      const b = BUILDINGS.find((x) => x.id === p.get('b'));
      const ho = +(p.get('h') || 1);
      const top = (b.stepTop && b.stepTop[ho]) || b.floors;
      const f = snapAwayFromRefuge(+p.get('f'), top);
      $('floorSlider').value = String(f);
      $('floorLabel').textContent = `${f}층`;
      state.lastFloor = f;
    }
...
// ────────────────────────────────────────────────────────────
// 13. 렌더 루프 — 변경이 있을 때만 실제 그리기
// ────────────────────────────────────────────────────────────
let last = performance.now();
function animate(now) {
  requestAnimationFrame(animate);
  // 탭이 백그라운드면 아무것도 하지 않는다
  ...
  if (state.playing) {
    ...
    $('timeSlider').value = state.hour;
    $('timeLabel').textContent = formatHour(state.hour);
    // 재생 중: 밝기는 매 프레임, 그림자 맵은 updateSun 안에서 스로틀
    updateSun({ forceShadow: false });
    needsRender = true;
  }

  // 감쇠(damping)가 남아 있으면 update()가 true → 계속 그린다
...
frameSite();                     // 화면 비율에 맞게 카메라 거리 조정 (특히 세로 폰)
readUrl();                       // 공유 링크로 들어왔으면 그 상태로 복원
$('timeLabel').textContent = formatHour(state.hour);
updateLearnInfo();
updateSunPath();
updateSun({ forceShadow: true });
requestAnimationFrame(animate);

// 디버깅용
window.__app = {
  scene, camera, controls, state, towerMap, siteGroup, updateSun, requestRender,
...
addEventListener('resize', () => {
  ...
  // 폰을 가로/세로로 돌렸을 때만 거리를 다시 잡는다 (주소창 여닫힘엔 반응하지 않게)
